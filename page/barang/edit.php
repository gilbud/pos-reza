    ...
    <div class="card">
     <div class="body">
      <form method="POST">
        ...
        while ($data = $sql->fetch_assoc($sql)) {
          if ($tampil['id']==$data[id]) {
       ...
       <?php
       $tampil2=mysqli_query($koneksi,"SELECT * FROM tb_supplier");
       if ($tampil[id]==0){
      ...
      while($w=mysqli_fetch_array($tampil2)){
        ...
        if ($tampil[id]==$w[id]){
          ...
          echo "<option value=$w[id] selected>$w[nama_supplier]</option>";
        }

        else {
          echo "<option value=$w[id]>$w[nama_supplier]</option> </p> ";
        }
      }
      ?>                           
    </select>
...
<div class="form-group">
  <div class="form-line">
    <input type="number" name="stok" value="<?php echo $tampil['stok'];?>" class="form-control" readonly />
  </div>
</div>
...
if (isset($_POST['simpan'])) {
  ...
  $sql2 = $koneksi->query("update tb_barang set kode_barcode='$kode_barcode',nama_barang='$nama_barang',id='$id',satuan='$satuan',stok='$stok',harga_beli='$harga_beli',harga_jual='$harga_jual',profit='$profit' where kode_barcode='$kode_barcode2'");
